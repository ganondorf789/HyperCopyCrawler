...
import (
	...
	"github.com/hypercopy/crawler/internal/orders"
...
func main() {
	...
	_, cleanup, err := logger.Init("orders")
	if err != nil {
	...
	w := orders.NewWorker(db, proxyMgr, *workers, *delay)
	if err := w.Run(); err != nil {
		zap.S().Fatalf("run: %v", err)
	}

	zap.S().Info("[main] orders sync finished")
}
