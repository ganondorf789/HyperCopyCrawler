...
func (e *ExceedsLimitErr) Error() string {
	return fmt.Sprintf("orders exceed 2000 limit in 30s window [%d, %d], got %d", e.StartMs, e.EndMs, e.Count)
}

// FetchAllOrders 获取交易员从 startMs 到 endMs 的所有历史委托（自适应7层细分）
// 返回 *ExceedsLimitErr 表示30秒窗口仍超限，调用方应保存已获取数据并跳过该交易员
func FetchAllOrders(client *hyperliquid.Client, address string, startMs, endMs int64, delay time.Duration) ([]model.OrderEntry, *ExceedsLimitErr) {
...
// --- Level 1: 按月 ---
func fetchByMonth(client *hyperliquid.Client, address string, startMs, endMs int64, delay time.Duration) ([]model.OrderEntry, *ExceedsLimitErr) {
...
// --- Level 2: 按周 ---
func fetchByWeek(client *hyperliquid.Client, address string, startMs, endMs int64, delay time.Duration) ([]model.OrderEntry, *ExceedsLimitErr) {
	var all []model.OrderEntry
	cur := time.UnixMilli(startMs).UTC()
	end := time.UnixMilli(endMs).UTC()
...
// --- Level 3: 按天 ---
func fetchByDay(client *hyperliquid.Client, address string, startMs, endMs int64, delay time.Duration) ([]model.OrderEntry, *ExceedsLimitErr) {
	var all []model.OrderEntry
	cur := time.UnixMilli(startMs).UTC()
	end := time.UnixMilli(endMs).UTC()
...
// --- Level 4: 按小时 ---
func fetchByHour(client *hyperliquid.Client, address string, startMs, endMs int64, delay time.Duration) ([]model.OrderEntry, *ExceedsLimitErr) {
	var all []model.OrderEntry
	cur := time.UnixMilli(startMs).UTC()
	end := time.UnixMilli(endMs).UTC()
...
// --- Level 5: 按10分钟 ---
func fetchBy10Min(client *hyperliquid.Client, address string, startMs, endMs int64, delay time.Duration) ([]model.OrderEntry, *ExceedsLimitErr) {
	var all []model.OrderEntry
	cur := time.UnixMilli(startMs).UTC()
	end := time.UnixMilli(endMs).UTC()
...
// --- Level 6: 按2分钟 ---
func fetchBy2Min(client *hyperliquid.Client, address string, startMs, endMs int64, delay time.Duration) ([]model.OrderEntry, *ExceedsLimitErr) {
	var all []model.OrderEntry
	cur := time.UnixMilli(startMs).UTC()
	end := time.UnixMilli(endMs).UTC()
...
// --- Level 7: 按30秒（最细粒度） ---
func fetchBy30Sec(client *hyperliquid.Client, address string, startMs, endMs int64, delay time.Duration) ([]model.OrderEntry, *ExceedsLimitErr) {
	var all []model.OrderEntry
	cur := time.UnixMilli(startMs).UTC()
	end := time.UnixMilli(endMs).UTC()
