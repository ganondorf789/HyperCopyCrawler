...
type TraderFunding struct {
	...
	Address     string    `gorm:"type:varchar(42);not null;index:idx_tf_addr_time;comment:钱包地址"`
	Time        int64     `gorm:"not null;index:idx_tf_addr_time;comment:资金费时间（毫秒时间戳）"`
	Hash        string    `gorm:"type:varchar(66);not null;uniqueIndex;comment:交易哈希"`
